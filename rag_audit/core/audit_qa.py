...
class AuditQA:
    ...
    def answer_question(self, question_text: str, context: Optional[str] = None) -> AuditResponse:
        ...
        # Use RAG to retrieve relevant chunks from the document
        relevant_chunks = self.document_processor.retrieve_relevant_chunks(question.question, top_k=3)
        
        # Extract the document chunks and format them with their scores
        sources = []
        relevant_context = ""
        
        for i, (chunk, score) in enumerate(relevant_chunks):
            chunk_id = f"Chunk {i+1} (score: {score:.2f})"
            sources.append(chunk_id)
            relevant_context += f"--- {chunk_id} ---\n{chunk}\n\n"
